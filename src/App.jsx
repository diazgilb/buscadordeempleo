...
export default function App() {
  ...
  useEffect(() => {
    try {
      const p = localStorage.getItem("directorio-perfiles");
      setPerfiles(p ? JSON.parse(p) : []);
    } catch (e) {
      setPerfiles([]);
    }
    try {
      const v = localStorage.getItem("directorio-vacantes");
      setVacantes(v ? JSON.parse(v) : []);
    } catch (e) {
      setVacantes([]);
    }
    setLoading(false);
  ...
  function addPerfil(perfil) {
    const next = [...perfiles, perfil];
    setPerfiles(next);
    try {
      localStorage.setItem("directorio-perfiles", JSON.stringify(next));
      showToast("Tu perfil quedó publicado en este navegador.");
    } catch (e) {
      showToast("No se pudo guardar el perfil. Intenta de nuevo.");
    }
  }

  function addVacante(vac) {
    const next = [...vacantes, vac];
    setVacantes(next);
    try {
      localStorage.setItem("directorio-vacantes", JSON.stringify(next));
      showToast("Vacante publicada en este navegador.");
    } catch (e) {
    ...
    <div style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", background: "#E7EAE8", minHeight: "100%", color: "#1C2321" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative" }}>
          ...
          <div className="stack-mobile" style={{ background: "#fff", borderRadius: 12, padding: 14, display: "flex", gap: 10, boxShadow: "0 16px 40px rgba(0,0,0,0.28)", maxWidth: 900 }}>
            ...
            <div style={{ flex: 1.3, display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <input value={fTexto} onChange={(e) => setFTexto(e.target.value)} placeholder="Buscar por nombre u oficio..." style={{ border: "none", fontSize: 14.5, width: "100%", outline: "none" }} />
            </div>
          </div>
      ...
      <footer style={{ background: "#1C2321", padding: "24px 20px", textAlign: "center" }}>
        <p style={{ color: "#8A928C", fontSize: 12.5, margin: 0, maxWidth: 560, marginInline: "auto" }}>
          Versión inicial desplegada. Por ahora, lo que registres se guarda solo en tu propio navegador (no se comparte todavía entre visitantes) — el siguiente paso técnico es conectar una base de datos real (Supabase) para que todos vean el mismo directorio.
        </p>
      </footer>
